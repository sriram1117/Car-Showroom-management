    ...
    <div class="f">
        <form onsubmit="return login()" name="loginform" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="tel" class="form-control" id="phone" name="cell" placeholder="Enter your phone number">
                        </div>
                        <div class="form-group">
                            <label for="car_model">Select Car Model</label>
                            <select class="form-control" id="car_model" name="model">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control" name="bd" id="preferred_date">
                        </div>
                        <div class="form-group">
                            <label for="preferred_date">Preferred delivery Date</label>
                            <input type="date" class="form-control" name="dd" id="preferred_date">
                        </div>
                        <div class="form-group">
                            <label for="message">Additional Comments</label>
                            <textarea class="form-control" name="comment" id="message" rows="4"
...
pemail = form.getvalue("mail")
pcell = form.getvalue("cell")
pmodel = form.getvalue("model")
pbookingdate = form.getvalue("bd")
pdeliverydate = form.getvalue("dd")
pcomment = form.getvalue("comment")
psubmit = form.getvalue("sub")

if psubmit != None:
    q = """insert into booking_details(name,mail,phone,model,bdate,ddate,comment) 
               values('%s','%s','%s','%s','%s','%s','%s')""" % (
    pname, pemail, pcell, pmodel, pbookingdate, pdeliverydate, pcomment)
    cur.execute(q)
    conn.commit()
    ...
    toaddr = pemail
    subject = "car booking was successfully booked"
    body = """ NAME: '%s' \nMODEL: '%s' \nBOOKING DATE: '%s' \nDELIVERY DATE: '%s' """ % (
    pname, pmodel, pbookingdate, pdeliverydate)
    msg = """Subject:{}\n\n{}""".format(subject, body)
